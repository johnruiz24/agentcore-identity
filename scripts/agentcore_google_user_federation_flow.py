#!/usr/bin/env python3
"""
Start Google USER_FEDERATION OAuth through Bedrock AgentCore Identity.

This script:
1) Gets a Cognito access token accepted by the deployed gateway
2) Calls AgentCore Identity get_token(auth_flow=USER_FEDERATION)
3) Prints the exact authorization URL generated by AgentCore (with session binding)
4) Polls until token is available (after user completes consent)
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run AgentCore USER_FEDERATION OAuth flow")
    ...
    parser.add_argument(
        "--provider-name",
        default="google-oauth-client-q7jdi",
    )
    parser.add_argument(
        "--callback-url",
        default=(
            "https://bedrock-agentcore.eu-central-1.amazonaws.com/"
            "identities/oauth2/callback/2ea34625-6a3d-489a-9d76-9b059e82cf17"
        ),
    )
    parser.add_argument(
        "--provider-scope",
